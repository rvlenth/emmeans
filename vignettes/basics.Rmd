...
The results of `ref_grid()` or `emmeans()` (these are objects of class `emm`)
...
The other graphics option offered is the `plot()` method for `emm` objects. In
...
also be accessed via `vignette("`*name*`", "emmeans")`
  ...
  * Utilities for working with `emm` objects: ["utilities"](utilities.html)
