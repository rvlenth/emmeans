...
for example, we have four means (corrtesponding to the four combinations of `type` and `size`)
...
values indicate that small and medium cars are much closer together, and larger ones
are more separate -- as can be visualized in the earlier plot.
...
and uou can verify that these values are equal to the squares of the `t.ratio`s in the initial
