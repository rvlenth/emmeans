...
Much of what you do with the **emmeans** package involves these three basic steps:
  ...
  1. Run  `EMM <- emmeans(...)` to obtain estimates of means or marginal means
...
the same steps as for one factor using `treat` and then `dose` in the role of `treatment`. These analyses will yield the estimated *marginal* means for
each factor, and comparisons/contrasts thereof.

If you instead follow the procedures below for a model with interaction, you will find 
that the contrasts or comparisons, conditional on one factor, will all be the 
same as the marginal contrasts or comparisons described here.
...
obtain the cell means (for combinations of the two factors), and then compare
or contrasts them separately in each row and column of a tabular presentation
of the means
```r
EMM <- emmeans(model, ~ treat * dose)
EMM    # display the cell means

### pairwise comparisons within each row and column
pairs(EMM, by = "dose")    # compare treats for each dose
pairs(EMM, by = "treat")    # compare doses for each treat
```
...
If the "diagonal" comparisons (not withing the same row or column) are of
interest, you would do `pairs(EMM)` without a `by` variable. But you get a lot more
comparisons this way.

#### Interaction contrasts
Sometimes you may want to exampne *interaction contrasts*, which are contrasts of contrasts. The thing to know here is that `contrast()` or (`pairs()`) creates the same kind of object as `emmeans()`, so you can run them multiple time. For example,
```r
...
## Additional options {#options}
  1. See the help files for *both* `emmeans()` and `ref_grid()` for additional
     ...
     passed to `ref_grid()`.
  2. There are a number of [vignettes](index.html) provided with the package that include
...
### Having trouble with follow-up analyses
...
more complicated. The most common case is using a call like 
...
run it on just the first element, and issues a message that says so. 
