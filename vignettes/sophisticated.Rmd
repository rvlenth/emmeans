...
We subsequently obtain the reference gridsn for these models in the usual way.
For later use, we also fit the same mode with just the prior information.
...
cbpp_prior.rstan <- rstanarm::stan_glmer(
    cbind(incidence, size - incidence) ~ period + (1|herd) + (1|unit),
    data = cbpp, family = binomial, prior_PD = TRUE,
    prior = student_t(df = 5, location = 0, scale = 2, autoscale = FALSE),
