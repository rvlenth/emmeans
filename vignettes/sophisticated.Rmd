...
period.emm
```
The summary for EMMs of Bayesian models shows highest posterior density (HPD) intervals
rather than confidence intervals. Under the hood, the posterior sample of parameter estimates is
used to compute a corresponding sample of posterior EMMs, and it is those that are summarized.
(Technical note: the summary is actually rerouted to the `hpd.summary()` function, and it loads the **coda** package if it is not already loaded.
