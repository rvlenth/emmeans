...
the scaling parameters, but it can't always succeed. If it fails, 
...
we had standardized. Here is such an example.
```{r}
fiber.lm <- lm(strength ~ machine + diameter, data = fiber)

# On raw scale:
emmeans(fiber.lm, "machine")

# On standardized scale:
tran <- make.tran("scale", y = fiber$strength)
emmeans(fiber.lm, "machine", transform = tran)
```
