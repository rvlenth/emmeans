...
Some users prefer ratios of probabilities (risk ratios) rather than odds ratios.
These are easy to obtain. First, do
```{r}
...
The `neuralgia.glm` example takes advantage of special properties associated with the logit link.
If we have proportion data but some other link function is used in
fitting the model, we don't automatically get either odds ratios or risk ratios.
However, we can `regrid()` to the
`"log"` scale if we want risk ratios, or to the `"logit"` scale if we want odds
ratios. For example,
```{r}
