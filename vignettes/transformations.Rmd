...
For that reason, it can't be auto-detected; we need to explicitly specify the 
transformation using `make.tran()`. The methods are exactly as shown earlier in this
vignette, so we show the code but not the results for a hypothetical example.

One method is to fit the model and then add the transformation information later:
```{r, eval = FALSE}
mod <- lmer(scale(RT) ~ group + (1|subject), data = mydata)
...
mod <- with(make.tran("scale", y = mydata$RT),
            lmer(linkfun(RT) ~ group + (1|subject), data = mydata))
emmeans(mod, "group", type = "response")
```
...
You may even fake an exotic transformation, e.g., via `transform = make.tran("boxcox", 0.329)`.
...
model the count as a Poisson variable with (by default) a square-root link; and
