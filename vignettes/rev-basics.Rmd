...
<!-- THIS WILL REPLACE THE BASICS VIGNETTE LATER ON. LET'S PUT MORE
     EMPHASIS ON EXPERIMENTATION AND MODELING AND CUT OUT THE
     ARTIFICIAL STUFF.
     THE PIGS EXAMPLE REALLY SHOULD USE AN inverse TRANSFORMATION.
     IT HAPPENS THAT THE MARGINAL MEANS FOR percent STILL RANK DIFFERENTLY
     THAN THE EMMS. LET'S PUT THE emmeans CALCS FIRST, THEN DEMO THAT
     THESE REFLECT THE REF GRID NEEDED WITH THE EXPERIMENTAL FRAMEWORK
     WHILE THE MARGINAL MEANS DO NOT.
-->

## Contents {#contents}

  0. [Emphasis on experimental data](#experimentation)
...
## Emphasis on experimental data {#experimentation}
...
means -- and much of what the **emmeans** package offers -- relate to
*experimental* data, as opposed to *observational* data. In observational data,
...
This is important because it changes even what we mean by the term "marginal
means," a term that is part of this package's name. There is a clear theoretical
meaning to this term when applied to a population and observational data,
because we know what we mean by the joint distribution of the variables
involved, and we just compute an integral or a sum. With experimental data, 
the population, if you want to call it that, depends on what test conditions 
we impose. So typically, it comprises a grid of values corresponding to the test
conditions we actually choose (or chose) to use. If it is an experiment to
test a new drug or a new fertilizer or a new manufacturing process, the population
is something that never even existed beforehand!

Correspondingly, in your typical experimental-design course or textbook, you
spend a lot of time talking mostly about balanced experiments (equal sample
sizes) conducted on a regular array of treatments or treatment combinations.
And we compute marginal means by just averaging over the rows and columns
of this nice, balanced array of data values. Experimenters are comfortable
with that because that's what they studied. And those who don't experiment
might find this very strange, because they don't think of "the population"
as an abstract entity that is defined as the array of test conditions
in the experiment.
...
The ["AQuickStart" vignette](AQuickStart.html) provided with this package 
recommends that we start with a good model. Often, residual plots help with
this process. Let's look at residuals versus predicted for an initial model
with `conc` as the response:
```{r fig.height = 4.5, fig.alt = "Plot of residuals vs. predicted for a model with 'conc' as the response. It shows a fair amount of hereroscedasticity in the residuals."}
plot(lm(conc ~ source * factor(percent), data = pigs), which = 1)
```
The variability of residuals is considerably less for smaller predicted values
than for larger ones. It turns out that we get much more 
