...
method for `emm` objects. In the code below, we obtain the EMMs for `source` for
...
confusion. For one thing, the result is not an `emm` object anymore; it is a
`list` of `emm` objects, called an `emm_list`. You may need to be cognizant of
