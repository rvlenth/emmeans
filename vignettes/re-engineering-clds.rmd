...
would imply that `trt` and `trt2` must be equal, seemingly contradicting the
...
while two more disant ones do not differ significantly. If that happens, we
...
of grouping lines, in that each distinct symbol will spane a contiguous
...
it is the only mean in groiup `3`.
...
thanks to two things:
  ...
  1. The algorithm for making grouping letters is based on a matrix of Boolean
     ...
     any pair that is statistically different (thpose means must receive different
  ...
  2. There is such a thing as *equivalence testing*, by which we can establish
     ...
     spcified threshold $\delta$. One simple way to do this is to conduct *two
     ...
     $+\delta$. We can use rhis TOST method to set each Boolean pair to `FALSE` is
...
a distinctively different appearance, bacause common symbols tend to be
...
such tests, we have strong counclusions or *findings* -- those that have small
...
lines or symbols, those come across visyally as findings, and the problem with
...
version 0.1-8, `https://CRAN.R-project.org/package=multcompView`
 ...
 346--363.
  ...
  Least-Squares Means. R package version 1.8.1-90001,
