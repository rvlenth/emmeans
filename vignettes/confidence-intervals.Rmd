...
## Contents {#contents}
This vignette describes various ways of summarizing `emm` objects.

  1. [`emm` objects, described](#emmobj)
...
str(pigs.emm.s)
```
The most important method for `emm` objects is `summary()`. It is used as the
default for displaying an `emmeans()` result like `pigs.emm.s`. This `summary()`
method for `emm` objects) actually produces a `data.frame`, but with extra bells and
whistles:
```{r}
...
results just like you would access data in a data frame. The `emm` object itself
...
As you may have gathered, the most important method for `emm` objects is `summary()`.
...
specify a threshold value to use in a test of equivalence, noninferiority, or nonsuperiority. An equivalence test is kind of a backwards significance test, where differences enough smaller than `delta` are the ones that can be significant.  The help page for `summary.emm` gives the details of these tests.
...
## Graphics {#graphics}
Graphical displays of `emm` objects are described in the 
