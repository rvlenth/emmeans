...
require(MASS)
fmx <- robmixglm(formula = 100 * log10(pres) ~ bp, data = forbes, cores = 1)
```
...
```{r}
qdrg.robmixglm <- function(object, data = NULL, ...) {
    coef <- coef(object)
    idx <- seq_along(coef)
    qdrg(formula = formula(object), data = object$model, coef = coef,
...
```{r}
rg <- qdrg(object = fmx, at = list(bp = c(175, 200, 225)))
confint(rg, type = "response")
```
Be careful when using these methods; since `object` is not the first argument of
the generic `qdrg()` function, we need to explicitly specify `object =` in the
call to this method; otherwise, so hard-to-decipher error messages will be
thrown.

Package developers may provide "cheap" emmeans support by providing a `qdrg`
method like this. If you do this in your package, you should [export and
register the method](#exporting), and add `emmeans (>=1.12` and
