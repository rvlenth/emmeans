...
## Data example {#dataex}
The **MASS** package contains various functions that do robust or outlier-resistant model fitting. We will cobble together some **emmeans** support for these. But first, let's create a suitable dataset (a simulated two-factor experiment) for testing.\footnote{I unapologetically use `=` as the assignment operator. It is good enough for C and Java, and supported by R.}
```{r}
...
```{r}
emm_basis.lqs = function(object, trms, xlev, grid, ...) {                 #1
    m = model.frame(trms, grid, na.action = na.pass, xlev = xlev)         #2
    X = model.matrix(trms, m, contrasts.arg = object$contrasts)           #3
    bhat = coef(object)                                                   #4
    Xmat = model.matrix(trms, data=object$model)                          #5
    V = rev(object$scale)[1]^2 * solve(t(Xmat) %*% Xmat)                  #6
    nbasis = matrix(NA)                                                   #7
    dfargs = list(df = nrow(Xmat) - ncol(Xmat))                           #8
    dffun = function(k, dfargs) dfargs$df                                 #9
    list(X=X, bhat=bhat, nbasis=nbasis, V=V, dffun=dffun, dfargs=dfargs)  #10
}
```
...
Let's go through the listing of this method, line-by-line:
  ...
  * Lines 2--3: Construct the linear functions, `X`. This is a pretty standard 
    ...
    predictors, then pass it as data to `model.data` to create the associated
  ...
  * Lines 5--6: Obtain the covariance matrix, `V`, of `bhat`. In many models,
    ...
    Instead, I cobbled one together using what it would be for ordinary
    regression: $\hat\sigma^2(\mathbf{X}'\mathbf{X})^{-1}$, where $\mathbf{X}$ is
    the design matrix for the whole dataset (not the reference grid). Here,
    $\hat\sigma$ is obtained using the last element of the `scale` element of the
    object (depending on the method, there are one or two scale estimates). This
    probably under-estimates the variances and distorts the covariances, because
    robust estimators have some efficiency loss.
  * Line 7: Compute the basis for non-estimable functions. This applies only
    when there is a possibility of rank deficiency in the model, and `lqs` methods
    cannot handle that. All linear functions are estimable, and we signal that by
    setting `nbasis` equal to a $1\times1$ matrix of `NA`. If rank deficiency were
    ...
    have coded `nbasis = estimability::nonest.basis(Xmat)`. On the other hand,
    if rank-deficient cases are not possible, set `nbasis`
    equal to `all.estble`, a constant in the **estimability** package.
    
    There is a subtlety you need to know regarding estimability. Suppose the model
    is rank-deficient, so that the design matrix $\mathbf{X}$ has $p$ columns but
    rank $r<p$. In that case, `bhat` should be of length $p$ (not $r$), and there
    should be $p-r$ elements equal to `NA`, corresponding to columns of $\mathbf{X}$
    ...
    assessing estimability. `V` should be $r\times r$, however: the covariance
  ...
  * Lines 8--9: Obtain `dffun` and `dfargs`. This is a little awkward because it is     designed to allow support for mixed models, where approximate methods may be
    ...
    value. (Note: Some models, such as some generalized linear models, do not have a
    sensible way to determine degrees of freedom. In those cases, return `Inf`
    degrees of freedom, and asymptotic results will be computed.)
...
<!-- @index **emmeans** package!Exports for developers -->
...
A few additional functions are exported because they may be useful to developers. They are as follows:
  ...
  * `.all.vars(expr, retain)` Some users of your package may include `$` or
  ...
  * `.diag(x, nrow, ncol)` The base `diag` function has a booby trap whereby,
  ...
  * `.aovlist.dffun(k, dfargs)`] This function is exported because it is needed
  ...
  * `.get.offset(terms, grid)` If `terms` is a model formula containing an
    `offset` call, this is will compute that offset in the context of `grid` (a
    `data.frame`).
  * `.my.vcov(object, ...)` In a call to `ref_grid`, `emmeans`, etc., the user may
    ...
    substitute the user's `vcov.` when it is present in `...`.
...
### Exporting the methods {#exporting}
...
```
This only has an effect if the user has the **emmeans** package installed (in which case **estimability** is also installed, as it is required); otherwise the code is skipped. We need to import the prototypes for `recover_data` and `emm_basis`, and register our new methods.
