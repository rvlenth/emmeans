...
construct `emm` objects, including `ref_grid()`,
...
|lmerMod      |lme4     |[L](#L)  |`mode = c("kenward-roger", "satterthwaite",`                 |
|             |         |         |`"asymptotic")`. Also `emm_options(disable.pbkrtest =,`      |
|             |         |         |`lmer.df =, pbkrtest.limit =)`                               |
...
|             |         |         |`data` is required                                           |
|mer          |lme4.0   |[A](#A)  |May become obsolete                                          |
...
There is an optional `mode` argument that defaults to 
...
`get_emm_option("pbkrtest.limit")`. If desired, the user may use 
`emm_options` to adjust this limit from the default of 3000.
