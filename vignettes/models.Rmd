...
|averaging    |MuMIn    |[I](#I)  |`data` required unless `$data` added to the object           |
...
Note that class of the models that are combined determines what optional arguments 
may be available and whether `data` is required.
...
The optional `mode` argument affects the degrees of freedom. There is an
...
experimentally by randomly perturbing the response values. Thus, the degrees of
freedom will vary slightly (or possibly even a lot) if the reference grid is
re-calculated. The alternative method is `"df.error"` (for `gls`) and
