...
For **stanreg** objects, if the optional argument `use_pp` is set to `TRUE`, then
`rstanarm::posterior_predict()` is used to simulate predictions on the reference grid.
This choice is necessary if any smoothers are used; but may be preferable 
in other cases as well. The default for `use_pp` is `TRUE` for models fitted 
with `stan_gamm4`, and `FALSE` otherwise.
