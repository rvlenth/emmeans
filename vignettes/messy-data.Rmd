...
returned `emm` object. That would not be the case if there had been no `by`
variable.
...
Here are some ways you may notice mistakenly-identified nesting:
  ...
  * A `str()` listing of the `emm` object shows a nesting component
  * An `emmeans()` summary unexpectedly includes one or more factors
    that you didn't specify
  * There are `NA`s or `NonEst` cases where there should be estimates
