...
Run `lintr::lint_dir(path = c("R", "tests")` to check your style. A .lintr file is included in this directory for emmeans custom style. 
...
Generative AI (large language models, chatbots, agents, etc) is a useful tool for generating code, but there are serious concerns regarding quality of code that is generated by genAI, energy usage required for genAI tools, and copyright violations to construct large linear models. 

We have found it sufficiently useful thus that AI-assisted bug checking is welcome. At this time, fully AI-generated code (including unit tests) are not acceptable due to unpredictable quality. Perhaps LMMs (at least the cheap ones we can access) will improve in quality and we can visit this policy, but at this time, we want a human hand engaged in improving emmeans. 
