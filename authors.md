# Authors and Citation

## Authors

- **Russell V. Lenth**. Author, copyright holder.

- **Julia Piaskowski**. Maintainer, author.

- **Balazs Banfai**. Contributor.

- **Ben Bolker**. Contributor.

- **Paul Buerkner**. Contributor.

- **Iago Giné-Vázquez**. Contributor.

- **Maxime Hervé**. Contributor.

- **Maarten Jung**. Contributor.

- **Jonathon Love**. Contributor.

- **Fernando Miguez**. Contributor.

- **Hannes Riebl**. Contributor.

- **Henrik Singmann**. Contributor.

## Citation

Source:
[`DESCRIPTION`](https://github.com/rvlenth/emmeans/blob/master/DESCRIPTION)

Lenth R, Piaskowski J (2025). *emmeans: Estimated Marginal Means, aka
Least-Squares Means*. R package version 2.0.1,
<https://rvlenth.github.io/emmeans/>.

    @Manual{,
      title = {emmeans: Estimated Marginal Means, aka Least-Squares Means},
      author = {Russell V. Lenth and Julia Piaskowski},
      year = {2025},
      note = {R package version 2.0.1},
      url = {https://rvlenth.github.io/emmeans/},
    }
